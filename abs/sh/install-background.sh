...
# 安装 vscode-background：dist/<name>-<version>.vsix → cursor/code
...
用法: $(basename "$0") [选项]
  ...
  默认：优先 dist/<name>-<package.json.version>.vsix；
        否则取 dist/ 里 semver 最大的 <name>-*.vsix；
        若无 vsix 则先打包再安装。

选项:
  --pack          强制先执行 pack-background.sh 再安装
...
示例:
  $(basename "$0")
  $(basename "$0") --pack
  $(basename "$0") --vsix "$OUT_DIR/background-2.0.4.vsix"
...
pick_vsix() {
  ...
  expected="$OUT_DIR/${PKG_NAME}-${version}.vsix"
  if [[ -f "$expected" ]]; then
    echo "$expected"
    return
  fi
  local latest=""
  shopt -s nullglob
  local files=("$OUT_DIR"/"${PKG_NAME}"-*.vsix)
  shopt -u nullglob
  if [[ ${#files[@]} -eq 0 ]]; then
    echo ""
    return
  fi
  latest="$(printf '%s\n' "${files[@]##*/}" | sed "s/^${PKG_NAME}-//;s/\\.vsix\$//" | sort -V | tail -1)"
  echo "$OUT_DIR/${PKG_NAME}-${latest}.vsix"
}
...
if [[ "$PACK" -eq 1 ]]; then
  bash "$ROOT/abs/sh/pack-background.sh"
fi
...
if [[ -z "$VSIX" || ! -f "$VSIX" ]]; then
  echo "找不到可安装的 vsix（期望 $OUT_DIR/${PKG_NAME}-*.vsix）" >&2
  exit 1
fi
