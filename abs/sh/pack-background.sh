...
mkdir -p "$OUT_DIR"
echo "→ vsce package"
# 必须打包 dependencies（sudo-prompt / stylis / uglify-js / lockfile），否则扩展激活失败
npx --no-install vsce package -o "$VSIX"

SIZE="$(du -h "$VSIX" | awk '{print $1}')"
echo "→ $VSIX ($SIZE)"
echo ""
echo "安装："
echo "  abs/sh/install-background.sh"
